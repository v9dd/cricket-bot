...
index e020b4274391091cbc7e9226b810aee3dbe6f207..679cb7910780660858b76e3c3d58c9ca36398a09 100644
--- a/bot.py
+++ b/bot.py
@@ -1,540 +1,762 @@
-import requests
...
+def get_pro_edit(text: str, team_batting: str | None = None) -> str | None:
...
-    if not text: return
+def send_telegram(text: str, pro_edit: bool = False, team_batting: str | None = None) -> None:
...
+
+
+def get_img_link(query: str) -> str:
...
+    """Convert cricket overs notation (e.g., 10.3) to total balls."""
+    if not overs:
+        return 0
...
+
+
+def stable_event_suffix(text: str) -> str:
+    return hashlib.sha1(text.encode("utf-8")).hexdigest()[:10]
+
+
+def is_international_text_check(text: str) -> bool:
     title = text.upper()
-    if any(x in title for x in [" U19", "TROPHY", "LEAGUE", " XI", "INDIA A", "PAKISTAN A", "ENGLAND LIONS"]): return False
+    if any(
+        x in title
+        for x in [" U19", "TROPHY", "LEAGUE", " XI", "INDIA A", "PAKISTAN A", "ENGLAND LIONS"]
+    ):
+        return False
...
+
 ...
 # =====================
-# SCRAPING ENGINE (ULTIMATE FILTER)
+# SCRAPING ENGINE
 # =====================
...
-                
-                # 1. Must be an International Match
-                if name and is_international_text_check(name):
-                    
-                    # 2. ULTIMATE GATEKEEPER: Check the parent card's status.
-                    # This ensures we NEVER track matches that are already over.
-                    parent_div = a_tag.find_parent()
-                    parent_text = parent_div.get_text(separator=" ", strip=True).lower() if parent_div else ""
-                    
-                    # If it contains "won by", "drawn", "tied", or "abandoned", SKIP IT ENTIRELY.
-                    if any(x in parent_text for x in ["won by", "win by", "match drawn", "match tied", "abandoned", "no result"]):
-                        continue 
-                    
...
+
             ...
             conn.commit()
-            return # Exit completely to avoid spam
+            return
...
-        pass
+        logger.warning("fetch_match_update failed for %s: %s", match_url, e)
+
+
+def run_bot():
...
-    send_telegram("✅ *Live-Only Tracker Active!* 🏏\n- Zero spam guaranteed.\n- Only actively playing matches will be monitored.")
-    
     while True:
         try:
             handle_commands()
